...
<body>
    ...
    <div class="container mt-4">
        ...
        <a href="${pageContext.request.contextPath}/admin/practice/add" class="btn btn-primary mb-2">Add New Practice</a>
        <c:if test="${empty practiceModels }">
            <div class="alert alert-info">
                No practices found.
            </div>
        </c:if>
        <c:if test="${not empty practiceModels }">    
        <table class="table table-bordered">
            ...
            <tbody>
            <c:forEach var="practice" items="${practiceModels}">
                <tr>
                    <td>${practice.practiceId}</td>
                    <td>${practice.practiceCode}</td>
                    <td>${practice.practiceName}</td>
                    <td>${practice.practiceStatus}</td>
                    <td>${practice.createdBy}</td>
                    <td>${practice.modifiedBy}</td>
                    <td>
                        <fmt:formatDate value="${practice.createdDate}" pattern="dd-MM-yyyy" var="createdDate" />
                        ${createdDate}
                    </td>
                    <td>
                        <fmt:formatDate value="${practice.modifiedDate}" pattern="dd-MM-yyyy" var="modifiedDate" />
                        ${modifiedDate}
                    </td>
                    <td>
                        <a href="${pageContext.request.contextPath}/admin/practice/edit/${practice.practiceId}" class="btn btn-sm btn-warning">Edit</a>
                        <a href="${pageContext.request.contextPath}/admin/practice/delete/${practice.practiceId}" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure?')">Delete</a>
                    </td>
                </tr>
            </c:forEach>
            </tbody>

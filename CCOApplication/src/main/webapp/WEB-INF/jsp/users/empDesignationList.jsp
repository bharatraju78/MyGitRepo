...
<div class="container mt-4">
        ...
        <table class="table table-bordered">
            ...
            <tbody>
            <c:if test="${empty empDesgs}">
                <tr>
                    <td colspan="5" class="text-center">No Designation History Found</td>
                </tr>
            </c:if>
            <c:if test="${not empty empDesgs}">    
            <c:forEach var="desg" items="${empDesgs}">
                <tr>
                    <td>${desg.designationId}</td>
                    <td>${desg.designationName}</td>
                    <td>
                    	<fmt:formatDate value="${desg.startDate}" pattern="dd-MM-yyyy" var="startDate" />
                    	${startDate}
                    </td>
                    <td>
                    	<fmt:formatDate value="${desg.endDate}" pattern="dd-MM-yyyy" var="endDate" />
                    	${endDate}
                    </td>
                    <td>${desg.comments}</td>
                </tr>
